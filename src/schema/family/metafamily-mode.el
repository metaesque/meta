...
;;;   instantiation of the template, not the template itself!
 ;;
 ;;      This is the file for Meta(Family) with suffix .metafamily
 ;;
...
;;;   least metafamily2-indent-offset more than the start-of-block line, but
;;;   if the line already has more indentation than that, it should be
;;;   left as-is.
;;;
;;; With respect to coloring, a simple set of rules is:
;;;   1) All construct names are colored with font-lock-metafamily2-construct-face
;;;   2) All attribute keys are colored with font-lock-metafamily2-attribute-key-face
;;;   3) All feature values are colored with font-lock-metafamily2-feature-value-face
;;;
...
; (defconst metafamily2-default-font "-*-Courier New-normal-normal-normal-*-11-*-*-*-m-0-iso10646-1")
(defconst metafamily2-default-font "-*-PT Mono-normal-normal-normal-*-10-*-*-*-m-0-iso10646-1")

(defconst metafamily2-constructs     '("Attribute" "BaseLanguage" "Construct" "FeatureValue" "File" "MetaLanguage" "Template" "event" "family" "link" "mating" "person"))
(defconst metafamily2-attribute-keys '("#" "#<*>" "%" "%<*>" ":<*>" "<" "<*>" "<<*>" "=" "=<*>" "@" "@<*>" "abbrev" "abbrev<*>" "alias" "alias<*>" "aliases" "aliases<*>" "associations" "associations<*>" "assocs" "assocs<*>" "autokey" "autokey<*>" "children" "children<*>" "cid" "cid<*>" "clsname" "clsname<*>" "color" "color<*>" "comment" "comment<*>" "compile" "compile<*>" "config" "config<*>" "default" "default<*>" "delim" "delim<*>" "dob" "dob<*>" "expand" "expand<*>" "female" "female<*>" "gender" "import" "import<*>" "key" "kind" "male" "male<*>" "name" "name<*>" "parent" "parent<*>" "pclsname" "pclsname<*>" "presence" "replacer" "replacer<*>" "scope" "scope<*>" "suffixes" "suffixes<*>" "text" "text<*>" "to" "to<*>" "toplevel" "toplevel<*>" "translate" "translate<*>" "type" "type<*>" "value" "when" "when<*>" "where" "where<*>" "who" "who<*>"))
(defconst metafamily2-feature-values '("abstract" "aliaskey" "concrete" "death" "divorce" "feature" "female" "male" "marriage" "nokey" "nongendered" "noval" "primary" "secondary" "showkey" "showval" "trans" "undef" "unknown" "userval"))
(defconst metafamily2-keywords       '("complex" "enum" "expr" "id" "num" "simple" "str" "type" "word" "xid"))
(defconst metafamily2-basewords      '())

(defconst metafamily2-constructs-re     (regexp-opt metafamily2-constructs t))
(defconst metafamily2-attribute-keys-re (regexp-opt metafamily2-attribute-keys t))
(defconst metafamily2-feature-values-re (regexp-opt metafamily2-feature-values t))
(defconst metafamily2-keywords-re       (regexp-opt metafamily2-keywords t))
(defconst metafamily2-basewords-re      (regexp-opt metafamily2-basewords t))

(setq RE (make-hash-table))
(puthash 'metafamily2-Attribute-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("aliaskey" "feature" "key" "kind" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-Attribute-line (concat (gethash 'metafamily2-Attribute-kv RE) "\\(Attribute\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-BaseLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-BaseLanguage-line (concat (gethash 'metafamily2-BaseLanguage-kv RE) "\\(BaseLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-Construct-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-Construct-line (concat (gethash 'metafamily2-Construct-kv RE) "\\(Construct\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-FeatureValue-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-FeatureValue-line (concat (gethash 'metafamily2-FeatureValue-kv RE) "\\(FeatureValue\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-File-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-File-line (concat (gethash 'metafamily2-File-kv RE) "\\(File\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-MetaLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-MetaLanguage-line (concat (gethash 'metafamily2-MetaLanguage-kv RE) "\\(MetaLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-Template-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-Template-line (concat (gethash 'metafamily2-Template-kv RE) "\\(Template\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-event-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("death" "divorce" "kind" "marriage" "unknown")) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-event-line (concat (gethash 'metafamily2-event-kv RE) "\\(event\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-family-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-family-line (concat (gethash 'metafamily2-family-kv RE) "\\(family\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-link-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-link-line (concat (gethash 'metafamily2-link-kv RE) "\\(link\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-mating-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-mating-line (concat (gethash 'metafamily2-mating-kv RE) "\\(mating\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-person-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("female" "gender" "male" "nongendered" "trans")) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-person-line (concat (gethash 'metafamily2-person-kv RE) "\\(person\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafamily2-all-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "aliaskey" "concrete" "death" "divorce" "feature" "female" "gender" "key" "kind" "male" "marriage" "nokey" "nongendered" "noval" "presence" "primary" "secondary" "showkey" "showval" "trans" "undef" "unknown" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metafamily2-construct-line (concat (gethash 'metafamily2-all-kv RE) "\\(" metafamily2-constructs-re "\\)[ \t]+\\([^ \t]+\\)") RE)

(defconst metafamily2-comment-start-re "\\(?:comment\\|#\\):\n")
(defconst metafamily2-comment-re "\\(?:comment\\|#\\):\n\\([ \t]+\\)\\(.*\n\\(\\1.*\n\\)*\\)")

; Various methods set these variables
(setq metafamily2-current-construct-kind nil)
(if (not (boundp 'metafamily2-meta-binary))
  (setq metafamily2-meta-binary "meta2"))

(defun metafamily2-goto-construct-line (&optional target-dent)
  ...
  (let* ((p (point))
         ...
         (cons-re (substring (gethash 'metafamily2-construct-line RE) 1))
    ...
    (setq metafamily2-current-construct-kind (match-string 3))
    (message "Construct %s" metafamily2-current-construct-kind)
    (forward-char dent)
    (point)
  )
)

(defun metafamily2-current-block ()
  ...
  (let* ((p (point))
         ...
         (cons-start (metafamily2-goto-construct-line))
...
(defun metafamily2-toggle-block (region &optional force)
...
(defun metafamily2-hide-blocks (key-pattern &optional lang-pattern)
  ...
  (save-excursion
    ...
    (let ((re (concat "\n.*" "\\(" key-pattern "\\)" "\\(<" lang-pattern ">\\)?:$")))
      ...
      (while (re-search-forward re nil t)
        (metafamily2-toggle-current-block)))))

(defun metafamily2-hide-blocks-old (key-pattern &optional lang-pattern)
  ;; Hide all blocks in file whose secondary key matches pattern.
  ;;   key-pattern: re
  ;;     The secondary key to match
  ;;   lang-pattern: re
...
(defun metafamily2-hide-blocks-except (key-pattern lang-pattern)
...
(defun metafamily2-toggle-current-block (&optional force)
  ;; If current block is hidden, reveal it, else hide it.
  ;; If force is t, always hide, do not toggle.
  (interactive)
  (metafamily2-toggle-block (metafamily2-current-block) force))

(defun metafamily2-show-current-block ()
  (interactive)
  ...
  (let ((region (metafamily2-current-block)))
    (goto-char (car region))
    (push-mark (cdr region) nil t)))

(defun metafamily2-remove-overlays ()
  (interactive)
...
(defun metafamily2-next-construct (kind &optional backward)
  ;; Find the start/end of the next construct of give kind
  (if (null kind) (setq kind "construct"))
  (let* ((p (point))
         (re (gethash (intern (concat "metafamily2-" kind "-line")) RE))
...
(defun metafamily2-narrow-to-current-construct (&optional kind)
  (interactive)
  (cond
   (kind
      ...
      (next-line 1)
      (let ((data (metafamily2-next-construct kind 'backward)))
        (widen)
        (narrow-to-region
   ...
   (t
    ...
    (setq metafamily2-current-construct-kind nil)
    (metafamily2-goto-construct-line)
    (if metafamily2-current-construct-kind
        (metafamily2-narrow-to-current-construct metafamily2-current-construct-kind)))
  )
)
...
(defun metaoopl2-python-only ()
  (interactive)
  (metafamily2-hide-blocks-except "scope" "py\\|python"))

(defun metaoopl2-cpp-only ()
  (interactive)
  (metafamily2-hide-blocks-except "scope" "cpp\\|c\\+\\+"))
...
;;; **************************************************************
;; Service routines
(defun metafamily2-set-face (face foreground background font)
  (interactive "sFace: \nsForeground: \nsBackground: \n Font: ")
...
;; Users can call 'metafamily2-set-face' to customize the colors
...
(metafamily2-set-face 'font-lock-metaoopl2-class-face          "red"             nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metaoopl2-behavior-face       "red"             nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metaoopl2-method-face         "orange"          nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metaoopl2-field-face          "orange"          nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metafamily2-construct-face      "darkgreen"       nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metafamily2-attribute-key-face  "darkolivegreen"  nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metafamily2-feature-value-face  "hotpink4"        nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metafamily2-keyword-face        "deep pink"       nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metafamily2-baseword-face       "purple"          nil  metafamily2-default-font)
(metafamily2-set-face 'font-lock-metafamily2-end-face            "ivory4"          nil  metafamily2-default-font)

;; Users can define 'metafamily2-mode-hook' to get special functionality
;; when this mode is invoked.
(defvar metafamily2-mode-hook nil
  ""
)

;; Users can specify the indentation at each level
(defvar metafamily2-indent-offset 2
...
(defvar metafamily2-wrap-collapsed-block-ends t
...
;:TEMP
(setq metafamily2-wrap-collapsed-block-ends t)
;:ENDTEMP

(defun metafamily2-make-map ()
  (let ((metafamily2-mode-map (make-keymap))
        (space-map (make-sparse-keymap))
        )
    ;; Add key bindings here
    (define-key metafamily2-mode-map "\C-j" 'newline-and-indent)

    ;; Iniital bindings for \C-@
    ;;   some emacs versions don't bind \C-@ to Ctrl space, so we
    ;;   do both
    (define-key metafamily2-mode-map [?\C-\ ] space-map)
    (define-key metafamily2-mode-map "\C-@" space-map)
    ...
    ;(define-key space-map "vm" (lambda () (interactive) (find-file (concat (getenv "METAROOT") "/root/lib/emacs/metafamily2-mode.el"))))
    ;(define-key space-map "lm" (lambda () (interactive) (load-file (concat (getenv "METAROOT") "/root/lib/emacs/metafamily2-mode.el"))))
    ...
    ;(define-key metafamily2-mode-map [(control \,)] 'meta-toggle-prev-overlay-color)
    ;(define-key metafamily2-mode-map [(control \.)] 'meta-toggle-next-overlay-color)
    ;(define-key metafamily2-mode-map [(control \<)] 'meta-toggle-prev-overlay-visibility)
    ;(define-key metafamily2-mode-map [(control \>)] 'meta-toggle-next-overlay-visibility)
...
(defun metafamily2-compute-font-lock-keywords ()
  (let ((executable-constructs '("method" "initializer" "finalizer" "function" "lifecycle" "behavior" "receiver" "command"))
    ...
    (list
      ...
      ;; Color the "end" token.
      '("end\\( [^;\n]*\\)?;" . font-lock-metafamily2-end-face)
      ...
      (cons (concat "\\<" metafamily2-constructs-re     "\\>") font-lock-metafamily2-construct-face)
      (cons (concat "\\<" metafamily2-attribute-keys-re "\\>") font-lock-metafamily2-attribute-key-face)
      (cons (concat "\\<" metafamily2-feature-values-re "\\>") font-lock-metafamily2-feature-value-face)
      ;; metafamily2-level keywords
      (cons (concat "\\<" metafamily2-keywords-re "\\>") font-lock-metafamily2-keyword-face)
      ;; baselang-level keywords
      (cons (concat "\\<" metafamily2-basewords-re "\\>") font-lock-metafamily2-baseword-face)
...
(defvar metafamily2-font-lock-keywords (metafamily2-compute-font-lock-keywords))
; TEMP - force changes to font info into the var
(setq metafamily2-font-lock-keywords (metafamily2-compute-font-lock-keywords))
; ENDTEMP.
...
(setq metafamily2-mode-syntax-table
  (let ((metafamily2-mode-syntax-table (make-syntax-table)))

    (modify-syntax-entry ?- "w" metafamily2-mode-syntax-table)
    (modify-syntax-entry ?_ "w" metafamily2-mode-syntax-table)
    (modify-syntax-entry ?. "w" metafamily2-mode-syntax-table)

    ;; This gives C++-style comments (/* ... */ and //)
    ;; Do not yet know how to treat COMMENT { ... } as a comment
    ;;;;;(modify-syntax-entry ?/  ". 124b" metafamily2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?*  ". 23" metafamily2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?\n "> b" metafamily2-mode-syntax-table)
    ;; This gives perl-style comments ('#').
    ;;;;;(modify-syntax-entry ?#  "< b" metafamily2-mode-syntax-table)
    ...
    (cond
     ...
     (t
      (modify-syntax-entry ?/  ". 14" metafamily2-mode-syntax-table)
      (modify-syntax-entry ?#  ". 2b" metafamily2-mode-syntax-table)
      ;(modify-syntax-entry ?\n "> a" metafamily2-mode-syntax-table)
      (modify-syntax-entry ?\n "> b" metafamily2-mode-syntax-table)
      ...
      ;; are defined.
      ;;;;(modify-syntax-entry ?\' "\"" metafamily2-mode-syntax-table)
      ;;;;(modify-syntax-entry ?\" "\"" metafamily2-mode-syntax-table)
    ...
    metafamily2-mode-syntax-table
  )
  ;;"Syntax table for metafamily2-mode"
)

;; This method indents the current line as Meta code
(defun metafamily2-indent-line ()
         ...
         ( (looking-at "^[ \t]*}") ; Check for rule 2
           (save-excursion
             ...
             (if (looking-at ".*{[ \t]*$")
               ...
               (setq cur-indent (- (meta-current-indentation) metafamily2-indent-offset))
             )
...
;; The mode method
(defun metafamily2-mode ()
  "Major mode for editing Meta(Family) language files"
  (interactive)
  (kill-all-local-variables)
  (set-syntax-table metafamily2-mode-syntax-table)
  ...
  (setq metafamily2-mode-map (metafamily2-make-map))
  (use-local-map metafamily2-mode-map)

  (set (make-local-variable 'font-lock-defaults) '(metafamily2-font-lock-keywords))
  (set (make-local-variable 'indent-line-function) 'metafamily2-indent-line)
  ...
  (add-hook 'font-lock-extend-region-functions 'metafamily2-font-lock-extend-region)
  
  ;; This sets up comment info
  (set (make-local-variable 'comment-start) "/#")
  (set (make-local-variable 'comment-style) 'multi-line)

  (setq major-mode 'metafamily2-mode)
  (setq mode-name "Meta(Family)")
  (run-hooks 'metafamily2-mode-hook)
)
...
(defun metafamily2-font-lock-extend-region ()
  ; (message "Here in metafamily2-font-lock-extend-region")
  (save-excursion
    (goto-char font-lock-beg)
    (let* ((back-limit (- font-lock-beg 2000))
           (future-limit (+ font-lock-beg 2000))
           (found-point (re-search-backward metafamily2-comment-start-re back-limit t)))
      (if found-point
          (let ((last-point (re-search-forward metafamily2-comment-re future-limit t)))
            (if (and last-point (> last-point font-lock-end))
                (progn
                  (setq font-lock-end last-point)))
            (setq font-lock-beg found-point))))))

(provide 'metafamily2-mode)
...
(defvar metafamily2-minor-mode-map (make-sparse-keymap)
  "Keymap while metafamily2-minor-mode is active.")

(define-minor-mode metafamily2-minor-mode
  "A temporary minor mode to be activated."
  nil
  :lighter " MetaMinor"
  metafamily2-minor-mode-map)

(defun metafamily2-index-to-line ()
...
(define-key metafamily2-minor-mode-map (kbd "RET") 'metafamily2-index-to-line)

(defun metafamily2-create-index (&optional prefix)
  (interactive "P")
  (metafamily2-create-index-private nil prefix)
)

(defun metafamily2-create-filtered-index (&optional prefix filter)
  (interactive "PsRegexp: ")
  (metafamily2-create-index-private filter prefix)
)

(defun metafamily2-create-index-private (filter usenum)
  "Create an index.
  ...
  (let ((command
    ...
    (metafamily2-mode)
    (orgstruct-mode)
    (metafamily2-minor-mode 1)
    (goto-char (point-min))
    (next-line 1)
  )
)

(provide 'metafamily2-minor-mode)
