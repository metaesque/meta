...
;;;   instantiation of the template, not the template itself!
 ;;
 ;;      This is the file for Meta(Food) with suffix .metafood
 ;;
...
;;;   least metafood2-indent-offset more than the start-of-block line, but
;;;   if the line already has more indentation than that, it should be
;;;   left as-is.
;;;
;;; With respect to coloring, a simple set of rules is:
;;;   1) All construct names are colored with font-lock-metafood2-construct-face
;;;   2) All attribute keys are colored with font-lock-metafood2-attribute-key-face
;;;   3) All feature values are colored with font-lock-metafood2-feature-value-face
;;;
...
; (defconst metafood2-default-font "-*-Courier New-normal-normal-normal-*-11-*-*-*-m-0-iso10646-1")
...
(defconst metafood2-constructs-re     (regexp-opt metafood2-constructs t))
(defconst metafood2-attribute-keys-re (regexp-opt metafood2-attribute-keys t))
(defconst metafood2-feature-values-re (regexp-opt metafood2-feature-values t))
(defconst metafood2-keywords-re       (regexp-opt metafood2-keywords t))
(defconst metafood2-basewords-re      (regexp-opt metafood2-basewords t))

(setq RE (make-hash-table))
(puthash 'metafood2-Attribute-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("aliaskey" "feature" "key" "kind" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metafood2-Attribute-line (concat (gethash 'metafood2-Attribute-kv RE) "\\(Attribute\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-BaseLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-BaseLanguage-line (concat (gethash 'metafood2-BaseLanguage-kv RE) "\\(BaseLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-Construct-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metafood2-Construct-line (concat (gethash 'metafood2-Construct-kv RE) "\\(Construct\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-FeatureValue-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-FeatureValue-line (concat (gethash 'metafood2-FeatureValue-kv RE) "\\(FeatureValue\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-File-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-File-line (concat (gethash 'metafood2-File-kv RE) "\\(File\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-MetaLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-MetaLanguage-line (concat (gethash 'metafood2-MetaLanguage-kv RE) "\\(MetaLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-Template-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-Template-line (concat (gethash 'metafood2-Template-kv RE) "\\(Template\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-day-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-day-line (concat (gethash 'metafood2-day-kv RE) "\\(day\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-exercise-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-exercise-line (concat (gethash 'metafood2-exercise-kv RE) "\\(exercise\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-food-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("certain" "certainty" "maybe" "wag")) "\[ \t\]\\)*") RE)
(puthash 'metafood2-food-line (concat (gethash 'metafood2-food-kv RE) "\\(food\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-meal-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metafood2-meal-line (concat (gethash 'metafood2-meal-kv RE) "\\(meal\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metafood2-all-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "aliaskey" "certain" "certainty" "concrete" "feature" "key" "kind" "maybe" "nokey" "noval" "presence" "primary" "secondary" "showkey" "showval" "undef" "userval" "value" "wag")) "\[ \t\]\\)*") RE)
(puthash 'metafood2-construct-line (concat (gethash 'metafood2-all-kv RE) "\\(" metafood2-constructs-re "\\)[ \t]+\\([^ \t]+\\)") RE)

(defconst metafood2-comment-start-re "\\(?:comment\\|#\\):\n")
(defconst metafood2-comment-re "\\(?:comment\\|#\\):\n\\([ \t]+\\)\\(.*\n\\(\\1.*\n\\)*\\)")

; Various methods set these variables
(setq metafood2-current-construct-kind nil)
(if (not (boundp 'metafood2-meta-binary))
  (setq metafood2-meta-binary "meta2"))

(defun metafood2-goto-construct-line (&optional target-dent)
  ...
  (let* ((p (point))
         ...
         (cons-re (substring (gethash 'metafood2-construct-line RE) 1))
    ...
    (setq metafood2-current-construct-kind (match-string 3))
    (message "Construct %s" metafood2-current-construct-kind)
    (forward-char dent)
    (point)
  )
)

(defun metafood2-current-block ()
  ...
  (let* ((p (point))
         ...
         (cons-start (metafood2-goto-construct-line))
...
(defun metafood2-toggle-block (region &optional force)
...
(defun metafood2-hide-blocks (key-pattern &optional lang-pattern)
  ...
  (save-excursion
    ...
    (let ((re (concat "\n.*" "\\(" key-pattern "\\)" "\\(<" lang-pattern ">\\)?:$")))
      ...
      (while (re-search-forward re nil t)
        (metafood2-toggle-current-block)))))

(defun metafood2-hide-blocks-old (key-pattern &optional lang-pattern)
  ;; Hide all blocks in file whose secondary key matches pattern.
  ;;   key-pattern: re
  ;;     The secondary key to match
  ;;   lang-pattern: re
...
(defun metafood2-hide-blocks-except (key-pattern lang-pattern)
...
(defun metafood2-toggle-current-block (&optional force)
  ;; If current block is hidden, reveal it, else hide it.
  ;; If force is t, always hide, do not toggle.
  (interactive)
  (metafood2-toggle-block (metafood2-current-block) force))

(defun metafood2-show-current-block ()
  (interactive)
  ...
  (let ((region (metafood2-current-block)))
    (goto-char (car region))
    (push-mark (cdr region) nil t)))

(defun metafood2-remove-overlays ()
  (interactive)
...
(defun metafood2-next-construct (kind &optional backward)
  ;; Find the start/end of the next construct of give kind
  (if (null kind) (setq kind "construct"))
  (let* ((p (point))
         (re (gethash (intern (concat "metafood2-" kind "-line")) RE))
...
(defun metafood2-narrow-to-current-construct (&optional kind)
  (interactive)
  (cond
   (kind
      ...
      (next-line 1)
      (let ((data (metafood2-next-construct kind 'backward)))
        (widen)
        (narrow-to-region
   ...
   (t
    ...
    (setq metafood2-current-construct-kind nil)
    (metafood2-goto-construct-line)
    (if metafood2-current-construct-kind
        (metafood2-narrow-to-current-construct metafood2-current-construct-kind)))
  )
)
...
(defun metaoopl2-python-only ()
  (interactive)
  (metafood2-hide-blocks-except "scope" "py\\|python"))

(defun metaoopl2-cpp-only ()
  (interactive)
  (metafood2-hide-blocks-except "scope" "cpp\\|c\\+\\+"))
...
;;; **************************************************************
;; Service routines
(defun metafood2-set-face (face foreground background font)
  (interactive "sFace: \nsForeground: \nsBackground: \n Font: ")
...
;; Users can call 'metafood2-set-face' to customize the colors
...
(metafood2-set-face 'font-lock-metaoopl2-class-face          "red"             nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metaoopl2-behavior-face       "red"             nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metaoopl2-method-face         "orange"          nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metaoopl2-field-face          "orange"          nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metafood2-construct-face      "darkgreen"       nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metafood2-attribute-key-face  "darkolivegreen"  nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metafood2-feature-value-face  "hotpink4"        nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metafood2-keyword-face        "deep pink"       nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metafood2-baseword-face       "purple"          nil  metafood2-default-font)
(metafood2-set-face 'font-lock-metafood2-end-face            "ivory4"          nil  metafood2-default-font)

;; Users can define 'metafood2-mode-hook' to get special functionality
;; when this mode is invoked.
(defvar metafood2-mode-hook nil
  ""
)

;; Users can specify the indentation at each level
(defvar metafood2-indent-offset 2
...
(defvar metafood2-wrap-collapsed-block-ends t
...
;:TEMP
(setq metafood2-wrap-collapsed-block-ends t)
;:ENDTEMP

(defun metafood2-make-map ()
  (let ((metafood2-mode-map (make-keymap))
        (space-map (make-sparse-keymap))
        )
    ;; Add key bindings here
    (define-key metafood2-mode-map "\C-j" 'newline-and-indent)

    ;; Iniital bindings for \C-@
    ;;   some emacs versions don't bind \C-@ to Ctrl space, so we
    ;;   do both
    (define-key metafood2-mode-map [?\C-\ ] space-map)
    (define-key metafood2-mode-map "\C-@" space-map)
    ...
    ;(define-key space-map "vm" (lambda () (interactive) (find-file (concat (getenv "METAROOT") "/root/lib/emacs/metafood2-mode.el"))))
    ;(define-key space-map "lm" (lambda () (interactive) (load-file (concat (getenv "METAROOT") "/root/lib/emacs/metafood2-mode.el"))))
    ...
    ;(define-key metafood2-mode-map [(control \,)] 'meta-toggle-prev-overlay-color)
    ;(define-key metafood2-mode-map [(control \.)] 'meta-toggle-next-overlay-color)
    ;(define-key metafood2-mode-map [(control \<)] 'meta-toggle-prev-overlay-visibility)
    ;(define-key metafood2-mode-map [(control \>)] 'meta-toggle-next-overlay-visibility)
...
(defun metafood2-compute-font-lock-keywords ()
  (let ((executable-constructs '("method" "initializer" "finalizer" "function" "lifecycle" "behavior" "receiver" "command"))
    ...
    (list
      ...
      (cons (concat "\\<" metafood2-constructs-re     "\\>") font-lock-metafood2-construct-face)
      (cons (concat "\\<" metafood2-attribute-keys-re "\\>") font-lock-metafood2-attribute-key-face)
      (cons (concat "\\<" metafood2-feature-values-re "\\>") font-lock-metafood2-feature-value-face)
      ;; metafood2-level keywords
      (cons (concat "\\<" metafood2-keywords-re "\\>") font-lock-metafood2-keyword-face)
      ;; baselang-level keywords
      (cons (concat "\\<" metafood2-basewords-re "\\>") font-lock-metafood2-baseword-face)
...
(defvar metafood2-font-lock-keywords (metafood2-compute-font-lock-keywords))
; TEMP - force changes to font info into the var
(setq metafood2-font-lock-keywords (metafood2-compute-font-lock-keywords))
; ENDTEMP.
...
(setq metafood2-mode-syntax-table
  (let ((metafood2-mode-syntax-table (make-syntax-table)))

    (modify-syntax-entry ?- "w" metafood2-mode-syntax-table)
    (modify-syntax-entry ?_ "w" metafood2-mode-syntax-table)
    (modify-syntax-entry ?. "w" metafood2-mode-syntax-table)

    ;; This gives C++-style comments (/* ... */ and //)
    ;; Do not yet know how to treat COMMENT { ... } as a comment
    ;;;;;(modify-syntax-entry ?/  ". 124b" metafood2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?*  ". 23" metafood2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?\n "> b" metafood2-mode-syntax-table)
    ;; This gives perl-style comments ('#').
    ;;;;;(modify-syntax-entry ?#  "< b" metafood2-mode-syntax-table)
    ...
    (cond
     ...
     (t
      (modify-syntax-entry ?/  ". 14" metafood2-mode-syntax-table)
      (modify-syntax-entry ?#  ". 2b" metafood2-mode-syntax-table)
      ;(modify-syntax-entry ?\n "> a" metafood2-mode-syntax-table)
      (modify-syntax-entry ?\n "> b" metafood2-mode-syntax-table)
      ...
      ;; are defined.
      ;;;;(modify-syntax-entry ?\' "\"" metafood2-mode-syntax-table)
      ;;;;(modify-syntax-entry ?\" "\"" metafood2-mode-syntax-table)
    ...
    metafood2-mode-syntax-table
  )
  ;;"Syntax table for metafood2-mode"
)

;; This method indents the current line as Meta code
(defun metafood2-indent-line ()
         ...
         ( (looking-at "^[ \t]*}") ; Check for rule 2
           (save-excursion
             ...
             (if (looking-at ".*{[ \t]*$")
               ...
               (setq cur-indent (- (meta-current-indentation) metafood2-indent-offset))
             )
...
;; The mode method
(defun metafood2-mode ()
  "Major mode for editing Meta(Food) language files"
  (interactive)
  (kill-all-local-variables)
  (set-syntax-table metafood2-mode-syntax-table)
  ...
  (setq metafood2-mode-map (metafood2-make-map))
  (use-local-map metafood2-mode-map)

  (set (make-local-variable 'font-lock-defaults) '(metafood2-font-lock-keywords))
  (set (make-local-variable 'indent-line-function) 'metafood2-indent-line)
  ...
  (add-hook 'font-lock-extend-region-functions 'metafood2-font-lock-extend-region)
  
  ;; This sets up comment info
  (set (make-local-variable 'comment-start) "/#")
  (set (make-local-variable 'comment-style) 'multi-line)

  (setq major-mode 'metafood2-mode)
  (setq mode-name "Meta(Food)")
  (run-hooks 'metafood2-mode-hook)
)
...
(defun metafood2-font-lock-extend-region ()
  ; (message "Here in metafood2-font-lock-extend-region")
  (save-excursion
    (goto-char font-lock-beg)
    (let* ((back-limit (- font-lock-beg 2000))
           (future-limit (+ font-lock-beg 2000))
           (found-point (re-search-backward metafood2-comment-start-re back-limit t)))
      (if found-point
          (let ((last-point (re-search-forward metafood2-comment-re future-limit t)))
            (if (and last-point (> last-point font-lock-end))
                (progn
                  (setq font-lock-end last-point)))
            (setq font-lock-beg found-point))))))

(provide 'metafood2-mode)
...
(defvar metafood2-minor-mode-map (make-sparse-keymap)
  "Keymap while metafood2-minor-mode is active.")

(define-minor-mode metafood2-minor-mode
  "A temporary minor mode to be activated."
  nil
  :lighter " MetaMinor"
  metafood2-minor-mode-map)

(defun metafood2-index-to-line ()
...
(define-key metafood2-minor-mode-map (kbd "RET") 'metafood2-index-to-line)

(defun metafood2-create-index (&optional prefix)
  (interactive "P")
  (metafood2-create-index-private nil prefix)
)

(defun metafood2-create-filtered-index (&optional prefix filter)
  (interactive "PsRegexp: ")
  (metafood2-create-index-private filter prefix)
)

(defun metafood2-create-index-private (filter usenum)
  "Create an index.
  ...
  (let ((command
    ...
    (metafood2-mode)
    (orgstruct-mode)
    (metafood2-minor-mode 1)
    (goto-char (point-min))
    (next-line 1)
  )
)

(provide 'metafood2-minor-mode)
