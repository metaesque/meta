...
;;;   instantiation of the template, not the template itself!
 ;;
 ;;      This is the file for Meta(Bio) with suffix .metabio
 ;;
...
;;;   least metabio2-indent-offset more than the start-of-block line, but
;;;   if the line already has more indentation than that, it should be
;;;   left as-is.
;;;
;;; With respect to coloring, a simple set of rules is:
;;;   1) All construct names are colored with font-lock-metabio2-construct-face
;;;   2) All attribute keys are colored with font-lock-metabio2-attribute-key-face
;;;   3) All feature values are colored with font-lock-metabio2-feature-value-face
;;;
...
; (defconst metabio2-default-font "-*-Courier New-normal-normal-normal-*-11-*-*-*-m-0-iso10646-1")
(defconst metabio2-default-font "-*-PT Mono-normal-normal-normal-*-10-*-*-*-m-0-iso10646-1")

(defconst metabio2-constructs     '("Attribute" "BaseLanguage" "Construct" "FeatureValue" "File" "MetaLanguage" "Template" "person" "source" "video"))
(defconst metabio2-attribute-keys '("#" "#<*>" ":<*>" "<" "<*>" "<<*>" "=" "=<*>" "aliases" "aliases<*>" "autokey" "autokey<*>" "brand" "brand<*>" "children" "children<*>" "clsname" "clsname<*>" "color" "color<*>" "comment" "comment<*>" "compile" "compile<*>" "config" "config<*>" "date" "date<*>" "default" "default<*>" "delim" "delim<*>" "dob" "dob<*>" "duration" "duration<*>" "expand" "expand<*>" "given" "given<*>" "import" "import<*>" "key" "kind" "name" "name<*>" "parent" "parent<*>" "presence" "rawuri" "rawuri<*>" "replacer" "replacer<*>" "scope" "scope<*>" "suffixes" "suffixes<*>" "surname" "surname<*>" "title" "title<*>" "toplevel" "toplevel<*>" "translate" "translate<*>" "type" "type<*>" "uri" "uri<*>" "value"))
(defconst metabio2-feature-values '("abstract" "aliaskey" "concrete" "feature" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval"))
(defconst metabio2-keywords       '("complex" "enum" "expr" "id" "num" "simple" "str" "type" "word" "xid"))
(defconst metabio2-basewords      '())

(defconst metabio2-constructs-re     (regexp-opt metabio2-constructs t))
(defconst metabio2-attribute-keys-re (regexp-opt metabio2-attribute-keys t))
(defconst metabio2-feature-values-re (regexp-opt metabio2-feature-values t))
(defconst metabio2-keywords-re       (regexp-opt metabio2-keywords t))
(defconst metabio2-basewords-re      (regexp-opt metabio2-basewords t))

(setq RE (make-hash-table))
(puthash 'metabio2-Attribute-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("aliaskey" "feature" "key" "kind" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metabio2-Attribute-line (concat (gethash 'metabio2-Attribute-kv RE) "\\(Attribute\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-BaseLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metabio2-BaseLanguage-line (concat (gethash 'metabio2-BaseLanguage-kv RE) "\\(BaseLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-Construct-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metabio2-Construct-line (concat (gethash 'metabio2-Construct-kv RE) "\\(Construct\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-FeatureValue-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-FeatureValue-line (concat (gethash 'metabio2-FeatureValue-kv RE) "\\(FeatureValue\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-File-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-File-line (concat (gethash 'metabio2-File-kv RE) "\\(File\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-MetaLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-MetaLanguage-line (concat (gethash 'metabio2-MetaLanguage-kv RE) "\\(MetaLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-Template-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-Template-line (concat (gethash 'metabio2-Template-kv RE) "\\(Template\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-person-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-person-line (concat (gethash 'metabio2-person-kv RE) "\\(person\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-source-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-source-line (concat (gethash 'metabio2-source-kv RE) "\\(source\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-video-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metabio2-video-line (concat (gethash 'metabio2-video-kv RE) "\\(video\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metabio2-all-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "aliaskey" "concrete" "feature" "key" "kind" "nokey" "noval" "presence" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metabio2-construct-line (concat (gethash 'metabio2-all-kv RE) "\\(" metabio2-constructs-re "\\)[ \t]+\\([^ \t]+\\)") RE)

(defconst metabio2-comment-start-re "\\(?:comment\\|#\\):\n")
(defconst metabio2-comment-re "\\(?:comment\\|#\\):\n\\([ \t]+\\)\\(.*\n\\(\\1.*\n\\)*\\)")

; Various methods set these variables
(setq metabio2-current-construct-kind nil)
(if (not (boundp 'metabio2-meta-binary))
  (setq metabio2-meta-binary "meta2"))

(defun metabio2-goto-construct-line (&optional target-dent)
  ...
  (let* ((p (point))
         ...
         (cons-re (substring (gethash 'metabio2-construct-line RE) 1))
    ...
    (setq metabio2-current-construct-kind (match-string 3))
    (message "Construct %s" metabio2-current-construct-kind)
    (forward-char dent)
    (point)
  )
)

(defun metabio2-current-block ()
  ...
  (let* ((p (point))
         ...
         (cons-start (metabio2-goto-construct-line))
...
(defun metabio2-toggle-block (region &optional force)
...
(defun metabio2-hide-blocks (key-pattern &optional lang-pattern)
  ...
  (save-excursion
    ...
    (let ((re (concat "\n.*" "\\(" key-pattern "\\)" "\\(<" lang-pattern ">\\)?:$")))
      ...
      (while (re-search-forward re nil t)
        (metabio2-toggle-current-block)))))

(defun metabio2-hide-blocks-old (key-pattern &optional lang-pattern)
  ;; Hide all blocks in file whose secondary key matches pattern.
  ;;   key-pattern: re
  ;;     The secondary key to match
  ;;   lang-pattern: re
...
(defun metabio2-hide-blocks-except (key-pattern lang-pattern)
...
(defun metabio2-toggle-current-block (&optional force)
  ;; If current block is hidden, reveal it, else hide it.
  ;; If force is t, always hide, do not toggle.
  (interactive)
  (metabio2-toggle-block (metabio2-current-block) force))

(defun metabio2-show-current-block ()
  (interactive)
  ...
  (let ((region (metabio2-current-block)))
    (goto-char (car region))
    (push-mark (cdr region) nil t)))

(defun metabio2-remove-overlays ()
  (interactive)
...
(defun metabio2-next-construct (kind &optional backward)
  ;; Find the start/end of the next construct of give kind
  (if (null kind) (setq kind "construct"))
  (let* ((p (point))
         (re (gethash (intern (concat "metabio2-" kind "-line")) RE))
...
(defun metabio2-narrow-to-current-construct (&optional kind)
  (interactive)
  (cond
   (kind
      ...
      (next-line 1)
      (let ((data (metabio2-next-construct kind 'backward)))
        (widen)
        (narrow-to-region
   ...
   (t
    ...
    (setq metabio2-current-construct-kind nil)
    (metabio2-goto-construct-line)
    (if metabio2-current-construct-kind
        (metabio2-narrow-to-current-construct metabio2-current-construct-kind)))
  )
)
...
(defun metaoopl2-python-only ()
  (interactive)
  (metabio2-hide-blocks-except "scope" "py\\|python"))

(defun metaoopl2-cpp-only ()
  (interactive)
  (metabio2-hide-blocks-except "scope" "cpp\\|c\\+\\+"))
...
;;; **************************************************************
;; Service routines
(defun metabio2-set-face (face foreground background font)
  (interactive "sFace: \nsForeground: \nsBackground: \n Font: ")
...
;; Users can call 'metabio2-set-face' to customize the colors
...
(metabio2-set-face 'font-lock-metaoopl2-class-face          "red"             nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metaoopl2-behavior-face       "red"             nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metaoopl2-method-face         "orange"          nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metaoopl2-field-face          "orange"          nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metabio2-construct-face      "darkgreen"       nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metabio2-attribute-key-face  "darkolivegreen"  nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metabio2-feature-value-face  "hotpink4"        nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metabio2-keyword-face        "deep pink"       nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metabio2-baseword-face       "purple"          nil  metabio2-default-font)
(metabio2-set-face 'font-lock-metabio2-end-face            "ivory4"          nil  metabio2-default-font)

;; Users can define 'metabio2-mode-hook' to get special functionality
;; when this mode is invoked.
(defvar metabio2-mode-hook nil
  ""
)

;; Users can specify the indentation at each level
(defvar metabio2-indent-offset 2
...
(defvar metabio2-wrap-collapsed-block-ends t
...
;:TEMP
(setq metabio2-wrap-collapsed-block-ends t)
;:ENDTEMP

(defun metabio2-make-map ()
  (let ((metabio2-mode-map (make-keymap))
        (space-map (make-sparse-keymap))
        )
    ;; Add key bindings here
    (define-key metabio2-mode-map "\C-j" 'newline-and-indent)

    ;; Iniital bindings for \C-@
    ;;   some emacs versions don't bind \C-@ to Ctrl space, so we
    ;;   do both
    (define-key metabio2-mode-map [?\C-\ ] space-map)
    (define-key metabio2-mode-map "\C-@" space-map)
    ...
    ;(define-key space-map "vm" (lambda () (interactive) (find-file (concat (getenv "METAROOT") "/root/lib/emacs/metabio2-mode.el"))))
    ;(define-key space-map "lm" (lambda () (interactive) (load-file (concat (getenv "METAROOT") "/root/lib/emacs/metabio2-mode.el"))))
    ...
    ;(define-key metabio2-mode-map [(control \,)] 'meta-toggle-prev-overlay-color)
    ;(define-key metabio2-mode-map [(control \.)] 'meta-toggle-next-overlay-color)
    ;(define-key metabio2-mode-map [(control \<)] 'meta-toggle-prev-overlay-visibility)
    ;(define-key metabio2-mode-map [(control \>)] 'meta-toggle-next-overlay-visibility)
...
(defun metabio2-compute-font-lock-keywords ()
  (let ((executable-constructs '("method" "initializer" "finalizer" "function" "lifecycle" "behavior" "receiver" "command"))
    ...
    (list
      ...
      (cons (concat "\\<" metabio2-constructs-re     "\\>") font-lock-metabio2-construct-face)
      (cons (concat "\\<" metabio2-attribute-keys-re "\\>") font-lock-metabio2-attribute-key-face)
      (cons (concat "\\<" metabio2-feature-values-re "\\>") font-lock-metabio2-feature-value-face)
      ;; metabio2-level keywords
      (cons (concat "\\<" metabio2-keywords-re "\\>") font-lock-metabio2-keyword-face)
      ;; baselang-level keywords
      (cons (concat "\\<" metabio2-basewords-re "\\>") font-lock-metabio2-baseword-face)
...
(defvar metabio2-font-lock-keywords (metabio2-compute-font-lock-keywords))
; TEMP - force changes to font info into the var
(setq metabio2-font-lock-keywords (metabio2-compute-font-lock-keywords))
; ENDTEMP.
...
(setq metabio2-mode-syntax-table
  (let ((metabio2-mode-syntax-table (make-syntax-table)))

    (modify-syntax-entry ?- "w" metabio2-mode-syntax-table)
    (modify-syntax-entry ?_ "w" metabio2-mode-syntax-table)
    (modify-syntax-entry ?. "w" metabio2-mode-syntax-table)

    ;; This gives C++-style comments (/* ... */ and //)
    ;; Do not yet know how to treat COMMENT { ... } as a comment
    ;;;;;(modify-syntax-entry ?/  ". 124b" metabio2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?*  ". 23" metabio2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?\n "> b" metabio2-mode-syntax-table)
    ;; This gives perl-style comments ('#').
    ;;;;;(modify-syntax-entry ?#  "< b" metabio2-mode-syntax-table)
    ...
    (cond
     ...
     (t
      (modify-syntax-entry ?/  ". 14" metabio2-mode-syntax-table)
      (modify-syntax-entry ?#  ". 2b" metabio2-mode-syntax-table)
      ;(modify-syntax-entry ?\n "> a" metabio2-mode-syntax-table)
      (modify-syntax-entry ?\n "> b" metabio2-mode-syntax-table)
      ...
      ;; are defined.
      ;;;;(modify-syntax-entry ?\' "\"" metabio2-mode-syntax-table)
      ;;;;(modify-syntax-entry ?\" "\"" metabio2-mode-syntax-table)
    ...
    metabio2-mode-syntax-table
  )
  ;;"Syntax table for metabio2-mode"
)

;; This method indents the current line as Meta code
(defun metabio2-indent-line ()
         ...
         ( (looking-at "^[ \t]*}") ; Check for rule 2
           (save-excursion
             ...
             (if (looking-at ".*{[ \t]*$")
               ...
               (setq cur-indent (- (meta-current-indentation) metabio2-indent-offset))
             )
...
;; The mode method
(defun metabio2-mode ()
  "Major mode for editing Meta(Bio) language files"
  (interactive)
  (kill-all-local-variables)
  (set-syntax-table metabio2-mode-syntax-table)
  ...
  (setq metabio2-mode-map (metabio2-make-map))
  (use-local-map metabio2-mode-map)

  (set (make-local-variable 'font-lock-defaults) '(metabio2-font-lock-keywords))
  (set (make-local-variable 'indent-line-function) 'metabio2-indent-line)
  ...
  (add-hook 'font-lock-extend-region-functions 'metabio2-font-lock-extend-region)
  
  ;; This sets up comment info
  (set (make-local-variable 'comment-start) "/#")
  (set (make-local-variable 'comment-style) 'multi-line)

  (setq major-mode 'metabio2-mode)
  (setq mode-name "Meta(Bio)")
  (run-hooks 'metabio2-mode-hook)
)
...
(defun metabio2-font-lock-extend-region ()
  ; (message "Here in metabio2-font-lock-extend-region")
  (save-excursion
    (goto-char font-lock-beg)
    (let* ((back-limit (- font-lock-beg 2000))
           (future-limit (+ font-lock-beg 2000))
           (found-point (re-search-backward metabio2-comment-start-re back-limit t)))
      (if found-point
          (let ((last-point (re-search-forward metabio2-comment-re future-limit t)))
            (if (and last-point (> last-point font-lock-end))
                (progn
                  (setq font-lock-end last-point)))
            (setq font-lock-beg found-point))))))

(provide 'metabio2-mode)
...
(defvar metabio2-minor-mode-map (make-sparse-keymap)
  "Keymap while metabio2-minor-mode is active.")

(define-minor-mode metabio2-minor-mode
  "A temporary minor mode to be activated."
  nil
  :lighter " MetaMinor"
  metabio2-minor-mode-map)

(defun metabio2-index-to-line ()
...
(define-key metabio2-minor-mode-map (kbd "RET") 'metabio2-index-to-line)

(defun metabio2-create-index (&optional prefix)
  (interactive "P")
  (metabio2-create-index-private nil prefix)
)

(defun metabio2-create-filtered-index (&optional prefix filter)
  (interactive "PsRegexp: ")
  (metabio2-create-index-private filter prefix)
)

(defun metabio2-create-index-private (filter usenum)
  "Create an index.
  ...
  (let ((command
    ...
    (metabio2-mode)
    (orgstruct-mode)
    (metabio2-minor-mode 1)
    (goto-char (point-min))
    (next-line 1)
  )
)

(provide 'metabio2-minor-mode)
