...
;;;   instantiation of the template, not the template itself!
 ;;
 ;;      This is the file for Meta(Meta) with suffix .metameta
 ;;
...
;;;   least metameta2-indent-offset more than the start-of-block line, but
;;;   if the line already has more indentation than that, it should be
;;;   left as-is.
;;;
;;; With respect to coloring, a simple set of rules is:
;;;   1) All construct names are colored with font-lock-metameta2-construct-face
;;;   2) All attribute keys are colored with font-lock-metameta2-attribute-key-face
;;;   3) All feature values are colored with font-lock-metameta2-feature-value-face
;;;
...
; (defconst metameta2-default-font "-*-Courier New-normal-normal-normal-*-11-*-*-*-m-0-iso10646-1")
(defconst metameta2-default-font "-*-PT Mono-normal-normal-normal-*-10-*-*-*-m-0-iso10646-1")

(defconst metameta2-constructs     '("Attribute" "BaseLanguage" "Construct" "FeatureValue" "File" "MetaLanguage" "Template"))
(defconst metameta2-attribute-keys '("#" "<" "=" "aliases" "autokey" "children" "comment" "config" "default" "delim" "key" "kind" "name" "parent" "presence" "replacer" "scope" "suffixes" "toplevel" "type" "value"))
(defconst metameta2-feature-values '("abstract" "aliaskey" "concrete" "feature" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval"))
(defconst metameta2-keywords       '("complex" "enum" "expr" "id" "num" "simple" "str" "type" "word" "xid"))
(defconst metameta2-basewords      '())

(defconst metameta2-constructs-re     (regexp-opt metameta2-constructs t))
(defconst metameta2-attribute-keys-re (regexp-opt metameta2-attribute-keys t))
(defconst metameta2-feature-values-re (regexp-opt metameta2-feature-values t))
(defconst metameta2-keywords-re       (regexp-opt metameta2-keywords t))
(defconst metameta2-basewords-re      (regexp-opt metameta2-basewords t))

(setq RE (make-hash-table))
(puthash 'metameta-Attribute-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("aliaskey" "feature" "key" "kind" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metameta-Attribute-line (concat (gethash 'metameta-Attribute-kv RE) "\\(Attribute\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-BaseLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metameta-BaseLanguage-line (concat (gethash 'metameta-BaseLanguage-kv RE) "\\(BaseLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-Construct-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metameta-Construct-line (concat (gethash 'metameta-Construct-kv RE) "\\(Construct\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-FeatureValue-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metameta-FeatureValue-line (concat (gethash 'metameta-FeatureValue-kv RE) "\\(FeatureValue\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-File-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metameta-File-line (concat (gethash 'metameta-File-kv RE) "\\(File\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-MetaLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metameta-MetaLanguage-line (concat (gethash 'metameta-MetaLanguage-kv RE) "\\(MetaLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-Template-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metameta-Template-line (concat (gethash 'metameta-Template-kv RE) "\\(Template\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metameta-all-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "aliaskey" "concrete" "feature" "key" "kind" "nokey" "noval" "presence" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metameta-construct-line (concat (gethash 'metameta-all-kv RE) "\\(" metameta-constructs-re "\\)[ \t]+\\([^ \t]+\\)") RE)

(defconst metameta2-comment-start-re "\\(?:comment\\|#\\):\n")
(defconst metameta2-comment-re "\\(?:comment\\|#\\):\n\\([ \t]+\\)\\(.*\n\\(\\1.*\n\\)*\\)")

; Various methods set these variables
(setq metameta2-current-construct-kind nil)

(defun metameta2-goto-construct-line (&optional target-dent)
  ...
  (let* ((p (point))
         ...
         (cons-re (substring (gethash 'metameta2-construct-line RE) 1))
    ...
    (setq metameta2-current-construct-kind (match-string 3))
    (message "Construct %s" metameta2-current-construct-kind)
    (forward-char dent)
    (point)
  )
)

(defun metameta2-current-block ()
  ...
  (let* ((p (point))
         ...
         (cons-start (metameta2-goto-construct-line))
...
(defun metameta2-toggle-block (region &optional force)
...
(defun metameta2-hide-blocks (key-pattern &optional lang-pattern)
  ...
  (save-excursion
    ...
    (let ((re (concat "\n.*" "\\(" key-pattern "\\)" "\\(<" lang-pattern ">\\)?:$")))
      ...
      (while (re-search-forward re nil t)
        (metameta2-toggle-current-block)))))

(defun metameta2-hide-blocks-old (key-pattern &optional lang-pattern)
  ;; Hide all blocks in file whose secondary key matches pattern.
  ;;   key-pattern: re
  ;;     The secondary key to match
  ;;   lang-pattern: re
...
(defun metameta2-hide-blocks-except (key-pattern lang-pattern)
...
(defun metameta2-toggle-current-block (&optional force)
  ;; If current block is hidden, reveal it, else hide it.
  ;; If force is t, always hide, do not toggle.
  (interactive)
  (metameta2-toggle-block (metameta2-current-block) force))

(defun metameta2-show-current-block ()
  (interactive)
  ...
  (let ((region (metameta2-current-block)))
    (goto-char (car region))
    (push-mark (cdr region) nil t)))

(defun metameta2-remove-overlays ()
  (interactive)
...
(defun metameta2-next-construct (kind &optional backward)
  ;; Find the start/end of the next construct of give kind
  (if (null kind) (setq kind "construct"))
  (let* ((p (point))
         (re (gethash (intern (concat "metameta2-" kind "-line")) RE))
...
(defun metameta2-narrow-to-current-construct (&optional kind)
  (interactive)
  (cond
   (kind
      ...
      (next-line 1)
      (let ((data (metameta2-next-construct kind 'backward)))
        (widen)
        (narrow-to-region
   ...
   (t
    ...
    (setq metameta2-current-construct-kind nil)
    (metameta2-goto-construct-line)
    (if metameta2-current-construct-kind
        (metameta2-narrow-to-current-construct metameta2-current-construct-kind)))
  )
)
...
;;; **************************************************************
;; Service routines
(defun metameta2-set-face (face foreground background font)
  (interactive "sFace: \nsForeground: \nsBackground: \n Font: ")
...
(metameta2-set-face 'font-lock-metameta2-keyword-face        "deep pink"       nil  metameta2-default-font)
(metameta2-set-face 'font-lock-metameta2-baseword-face       "purple"          nil  metameta2-default-font)
(metameta2-set-face 'font-lock-metameta2-end-face            "ivory4"          nil  metameta2-default-font)

;; Users can define 'metameta2-mode-hook' to get special functionality
;; when this mode is invoked.
(defvar metameta2-mode-hook nil
  ""
)

;; Users can specify the indentation at each level
(defvar metameta2-indent-offset 2
...
(defvar metameta2-wrap-collapsed-block-ends t
...
;:TEMP
(setq metameta2-wrap-collapsed-block-ends t)
;:ENDTEMP

(defun metameta2-make-map ()
  (let ((metameta2-mode-map (make-keymap))
        (space-map (make-sparse-keymap))
        )
    ;; Add key bindings here
    (define-key metameta2-mode-map "\C-j" 'newline-and-indent)

    ;; Iniital bindings for \C-@
    ;;   some emacs versions don't bind \C-@ to Ctrl space, so we
    ;;   do both
    (define-key metameta2-mode-map [?\C-\ ] space-map)
    (define-key metameta2-mode-map "\C-@" space-map)
    ...
    ;(define-key space-map "vm" (lambda () (interactive) (find-file (concat (getenv "METAROOT") "/root/lib/emacs/metameta2-mode.el"))))
    ...
    ;(define-key metameta2-mode-map [(control \,)] 'meta-toggle-prev-overlay-color)
    ;(define-key metameta2-mode-map [(control \.)] 'meta-toggle-next-overlay-color)
    ;(define-key metameta2-mode-map [(control \<)] 'meta-toggle-prev-overlay-visibility)
    ;(define-key metameta2-mode-map [(control \>)] 'meta-toggle-next-overlay-visibility)
...
(setq metameta2-mode-map
;;(defvar metameta2-mode-map
   (metameta2-make-map)
...
(defun metameta2-compute-font-lock-keywords ()
  (let ((executable-constructs '("method" "initializer" "finalizer" "function" "lifecycle" "behavior" "receiver"))
    ...
    (list
      ...
      (cons (concat "\\<" metameta2-attribute-keys-re "\\>") font-lock-metameta2-attribute-key-face)
      (cons (concat "\\<" metameta2-feature-values-re "\\>") font-lock-metameta2-feature-value-face)
      ;; metameta2-level keywords
      (cons (concat "\\<" metameta-keywords-re "\\>") font-lock-metameta-keyword-face)
      ;; baselang-level keywords
      (cons (concat "\\<" metameta-basewords-re "\\>") font-lock-metameta-baseword-face)
...
(defvar metameta2-font-lock-keywords (metameta2-compute-font-lock-keywords))
; TEMP - force changes to font info into the var
(setq metameta2-font-lock-keywords (metameta2-compute-font-lock-keywords))
; ENDTEMP.
...
(setq metameta2-mode-syntax-table
  (let ((metameta2-mode-syntax-table (make-syntax-table)))

    (modify-syntax-entry ?- "w" metameta2-mode-syntax-table)
    (modify-syntax-entry ?_ "w" metameta2-mode-syntax-table)
    (modify-syntax-entry ?. "w" metameta2-mode-syntax-table)

    ;; This gives C++-style comments (/* ... */ and //)
    ;; Do not yet know how to treat COMMENT { ... } as a comment
    ;;;;;(modify-syntax-entry ?/  ". 124b" metameta2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?*  ". 23" metameta2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?\n "> b" metameta2-mode-syntax-table)
    ;; This gives perl-style comments ('#').
    ;;;;;(modify-syntax-entry ?#  "< b" metameta2-mode-syntax-table)
    ...
    (cond
     ...
     (t
      (modify-syntax-entry ?/  ". 14" metameta2-mode-syntax-table)
      (modify-syntax-entry ?#  ". 2b" metameta2-mode-syntax-table)
      ;(modify-syntax-entry ?\n "> a" metameta2-mode-syntax-table)
      (modify-syntax-entry ?\n "> b" metameta2-mode-syntax-table)
      ...
      ;; are defined.
      ;;;;(modify-syntax-entry ?\' "\"" metameta2-mode-syntax-table)
      ;;;;(modify-syntax-entry ?\" "\"" metameta2-mode-syntax-table)
    ...
    metameta2-mode-syntax-table
  )
  ;;"Syntax table for metameta2-mode"
)

;; This method indents the current line as Meta code
(defun metameta2-indent-line ()
         ...
         ( (looking-at "^[ \t]*}") ; Check for rule 2
           (save-excursion
             ...
             (if (looking-at ".*{[ \t]*$")
               ...
               (setq cur-indent (- (meta-current-indentation) metameta2-indent-offset))
             )
...
;; The mode method
(defun metameta2-mode ()
  "Major mode for editing Meta(Meta) language files"
  (interactive)
  (kill-all-local-variables)
  (set-syntax-table metameta2-mode-syntax-table)
  ...
  (setq metameta2-mode-map (metameta2-make-map))
  (use-local-map metameta2-mode-map)

  (set (make-local-variable 'font-lock-defaults) '(metameta2-font-lock-keywords))
  (set (make-local-variable 'indent-line-function) 'metameta2-indent-line)
  ...
  (add-hook 'font-lock-extend-region-functions 'metameta2-font-lock-extend-region)
  
  ;; This sets up comment info
  (set (make-local-variable 'comment-start) "/#")
  (set (make-local-variable 'comment-style) 'multi-line)

  (setq major-mode 'metameta2-mode)
  (setq mode-name "Meta(Meta)")
  (run-hooks 'metameta2-mode-hook)
)
...
(defun metameta2-font-lock-extend-region ()
  ; (message "Here in metameta2-font-lock-extend-region")
  (save-excursion
    (goto-char font-lock-beg)
    (let* ((back-limit (- font-lock-beg 2000))
           (future-limit (+ font-lock-beg 2000))
           (found-point (re-search-backward metameta2-comment-start-re back-limit t)))
      (if found-point
          (let ((last-point (re-search-forward metameta2-comment-re future-limit t)))
            (if (and last-point (> last-point font-lock-end))
                (progn
                  (setq font-lock-end last-point)))
            (setq font-lock-beg found-point))))))

(provide 'metameta2-mode)
