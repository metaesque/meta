...
;;;   instantiation of the template, not the template itself!
 ;;
 ;;      This is the file for Meta(Story) with suffix .metastory
 ;;
...
;;;   least metastory2-indent-offset more than the start-of-block line, but
;;;   if the line already has more indentation than that, it should be
;;;   left as-is.
;;;
;;; With respect to coloring, a simple set of rules is:
;;;   1) All construct names are colored with font-lock-metastory2-construct-face
;;;   2) All attribute keys are colored with font-lock-metastory2-attribute-key-face
;;;   3) All feature values are colored with font-lock-metastory2-feature-value-face
;;;
...
(defconst metastory2-constructs-re     (regexp-opt metastory2-constructs t))
(defconst metastory2-attribute-keys-re (regexp-opt metastory2-attribute-keys t))
(defconst metastory2-feature-values-re (regexp-opt metastory2-feature-values t))
(defconst metastory2-keywords-re       (regexp-opt metastory2-keywords t))
(defconst metastory2-basewords-re      (regexp-opt metastory2-basewords t))

(setq RE (make-hash-table))
(puthash 'metastory2-Attribute-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("aliaskey" "feature" "key" "kind" "nokey" "noval" "primary" "secondary" "showkey" "showval" "undef" "userval" "value")) "\[ \t\]\\)*") RE)
(puthash 'metastory2-Attribute-line (concat (gethash 'metastory2-Attribute-kv RE) "\\(Attribute\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metastory2-BaseLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metastory2-BaseLanguage-line (concat (gethash 'metastory2-BaseLanguage-kv RE) "\\(BaseLanguage\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metastory2-Construct-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '("abstract" "concrete" "presence")) "\[ \t\]\\)*") RE)
(puthash 'metastory2-Construct-line (concat (gethash 'metastory2-Construct-kv RE) "\\(Construct\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metastory2-FeatureValue-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metastory2-FeatureValue-line (concat (gethash 'metastory2-FeatureValue-kv RE) "\\(FeatureValue\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metastory2-File-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
(puthash 'metastory2-File-line (concat (gethash 'metastory2-File-kv RE) "\\(File\\)[ \t]+\\([^ \t]+\\)") RE)
(puthash 'metastory2-MetaLanguage-kv (concat "\n\\([ \t]*\\)\\(" (regexp-opt '()) "\[ \t\]\\)*") RE)
...
(defconst metastory2-comment-re "\\(?:comment\\|#\\):\n\\([ \t]+\\)\\(.*\n\\(\\1.*\n\\)*\\)")
...
(defun metastory2-goto-construct-line (&optional target-dent)
  ...
  (let* ((p (point))
         ...
         (cons-re (substring (gethash 'metastory2-construct-line RE) 1))
    ...
    (setq metastory2-current-construct-kind (match-string 3))
    (message "Construct %s" metastory2-current-construct-kind)
    (forward-char dent)
    (point)
  )
)

(defun metastory2-current-block ()
  ...
  (let* ((p (point))
         ...
         (cons-start (metastory2-goto-construct-line))
...
(defun metastory2-toggle-block (region &optional force)
...
(defun metastory2-hide-blocks (key-pattern &optional lang-pattern)
  ...
  (save-excursion
    ...
    (let ((re (concat "\n.*" "\\(" key-pattern "\\)" "\\(<" lang-pattern ">\\)?:$")))
      ...
      (while (re-search-forward re nil t)
        (metastory2-toggle-current-block)))))

(defun metastory2-hide-blocks-old (key-pattern &optional lang-pattern)
  ;; Hide all blocks in file whose secondary key matches pattern.
  ;;   key-pattern: re
  ;;     The secondary key to match
  ;;   lang-pattern: re
...
(defun metastory2-hide-blocks-except (key-pattern lang-pattern)
...
(defun metastory2-toggle-current-block (&optional force)
  ;; If current block is hidden, reveal it, else hide it.
  ;; If force is t, always hide, do not toggle.
  (interactive)
  (metastory2-toggle-block (metastory2-current-block) force))

(defun metastory2-show-current-block ()
  (interactive)
  ...
  (let ((region (metastory2-current-block)))
    (goto-char (car region))
    (push-mark (cdr region) nil t)))

(defun metastory2-remove-overlays ()
  (interactive)
...
(defun metastory2-next-construct (kind &optional backward)
  ;; Find the start/end of the next construct of give kind
  (if (null kind) (setq kind "construct"))
  (let* ((p (point))
         (re (gethash (intern (concat "metastory2-" kind "-line")) RE))
...
(defun metastory2-narrow-to-current-construct (&optional kind)
  (interactive)
  (cond
   (kind
      ...
      (next-line 1)
      (let ((data (metastory2-next-construct kind 'backward)))
        (widen)
        (narrow-to-region
   ...
   (t
    ...
    (setq metastory2-current-construct-kind nil)
    (metastory2-goto-construct-line)
    (if metastory2-current-construct-kind
        (metastory2-narrow-to-current-construct metastory2-current-construct-kind)))
  )
)
...
(defun metaoopl2-python-only ()
  (interactive)
  (metastory2-hide-blocks-except "scope" "py\\|python"))

(defun metaoopl2-cpp-only ()
  (interactive)
  (metastory2-hide-blocks-except "scope" "cpp\\|c\\+\\+"))
...
;;; **************************************************************
;; Service routines
(defun metastory2-set-face (face foreground background font)
  (interactive "sFace: \nsForeground: \nsBackground: \n Font: ")
...
;; Users can call 'metastory2-set-face' to customize the colors
...
(metastory2-set-face 'font-lock-metaoopl2-class-face          "red"             nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metaoopl2-behavior-face       "red"             nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metaoopl2-method-face         "orange"          nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metaoopl2-field-face          "orange"          nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metastory2-construct-face      "darkgreen"       nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metastory2-attribute-key-face  "darkolivegreen"  nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metastory2-feature-value-face  "hotpink4"        nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metastory2-keyword-face        "deep pink"       nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metastory2-baseword-face       "purple"          nil  metastory2-default-font)
(metastory2-set-face 'font-lock-metastory2-end-face            "ivory4"          nil  metastory2-default-font)

;; Users can define 'metastory2-mode-hook' to get special functionality
;; when this mode is invoked.
(defvar metastory2-mode-hook nil
  ""
)

;; Users can specify the indentation at each level
(defvar metastory2-indent-offset 2
...
(defvar metastory2-wrap-collapsed-block-ends t
...
;:TEMP
(setq metastory2-wrap-collapsed-block-ends t)
;:ENDTEMP

(defun metastory2-make-map ()
  (let ((metastory2-mode-map (make-keymap))
        (space-map (make-sparse-keymap))
        )
    ;; Add key bindings here
    (define-key metastory2-mode-map "\C-j" 'newline-and-indent)

    ;; Iniital bindings for \C-@
    ;;   some emacs versions don't bind \C-@ to Ctrl space, so we
    ;;   do both
    (define-key metastory2-mode-map [?\C-\ ] space-map)
    (define-key metastory2-mode-map "\C-@" space-map)
    ...
    ;(define-key space-map "vm" (lambda () (interactive) (find-file (concat (getenv "METAROOT") "/root/lib/emacs/metastory2-mode.el"))))
    ;(define-key space-map "lm" (lambda () (interactive) (load-file (concat (getenv "METAROOT") "/root/lib/emacs/metastory2-mode.el"))))
    ...
    ;(define-key metastory2-mode-map [(control \,)] 'meta-toggle-prev-overlay-color)
    ;(define-key metastory2-mode-map [(control \.)] 'meta-toggle-next-overlay-color)
    ;(define-key metastory2-mode-map [(control \<)] 'meta-toggle-prev-overlay-visibility)
    ;(define-key metastory2-mode-map [(control \>)] 'meta-toggle-next-overlay-visibility)
...
(setq metastory2-mode-map
;;(defvar metastory2-mode-map
   (metastory2-make-map)
   ;;"Keymap for Meta major mode"
)

;; Establish a file-suffix to mode mapping
(add-to-list 'auto-mode-alist '("\\.metastory" . metastory2-mode))
...
(defun metastory2-compute-font-lock-keywords ()
  (let ((executable-constructs '("method" "initializer" "finalizer" "function" "lifecycle" "behavior" "receiver"))
    ...
    (list
      ...
      (cons (concat "\\<" metastory2-constructs-re     "\\>") font-lock-metastory2-construct-face)
      (cons (concat "\\<" metastory2-attribute-keys-re "\\>") font-lock-metastory2-attribute-key-face)
      (cons (concat "\\<" metastory2-feature-values-re "\\>") font-lock-metastory2-feature-value-face)
      ;; metastory2-level keywords
      (cons (concat "\\<" metastory2-keywords-re "\\>") font-lock-metastory2-keyword-face)
      ;; baselang-level keywords
      (cons (concat "\\<" metastory2-basewords-re "\\>") font-lock-metastory2-baseword-face)
...
(defvar metastory2-font-lock-keywords (metastory2-compute-font-lock-keywords))
; TEMP - force changes to font info into the var
(setq metastory2-font-lock-keywords (metastory2-compute-font-lock-keywords))
; ENDTEMP.
...
(setq metastory2-mode-syntax-table
  (let ((metastory2-mode-syntax-table (make-syntax-table)))

    (modify-syntax-entry ?- "w" metastory2-mode-syntax-table)
    (modify-syntax-entry ?_ "w" metastory2-mode-syntax-table)
    (modify-syntax-entry ?. "w" metastory2-mode-syntax-table)

    ;; This gives C++-style comments (/* ... */ and //)
    ;; Do not yet know how to treat COMMENT { ... } as a comment
    ;;;;;(modify-syntax-entry ?/  ". 124b" metastory2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?*  ". 23" metastory2-mode-syntax-table)
    ;;;;;(modify-syntax-entry ?\n "> b" metastory2-mode-syntax-table)
    ;; This gives perl-style comments ('#').
    ;;;;;(modify-syntax-entry ?#  "< b" metastory2-mode-syntax-table)
    ...
    (cond
     ...
     (t
      (modify-syntax-entry ?/  ". 14" metastory2-mode-syntax-table)
      (modify-syntax-entry ?#  ". 2b" metastory2-mode-syntax-table)
      ;(modify-syntax-entry ?\n "> a" metastory2-mode-syntax-table)
      (modify-syntax-entry ?\n "> b" metastory2-mode-syntax-table)
      ...
      ;; are defined.
      ;;;;(modify-syntax-entry ?\' "\"" metastory2-mode-syntax-table)
      ;;;;(modify-syntax-entry ?\" "\"" metastory2-mode-syntax-table)
    ...
    metastory2-mode-syntax-table
  )
  ;;"Syntax table for metastory2-mode"
)

;; This method indents the current line as Meta code
(defun metastory2-indent-line ()
         ...
         ( (looking-at "^[ \t]*}") ; Check for rule 2
           (save-excursion
             ...
             (if (looking-at ".*{[ \t]*$")
               ...
               (setq cur-indent (- (meta-current-indentation) metastory2-indent-offset))
             )
...
;; The mode method
(defun metastory2-mode ()
  "Major mode for editing Meta(Story) language files"
  (interactive)
  (kill-all-local-variables)
  (set-syntax-table metastory2-mode-syntax-table)
  ...
  (setq metastory2-mode-map (metastory2-make-map))
  (use-local-map metastory2-mode-map)

  (set (make-local-variable 'font-lock-defaults) '(metastory2-font-lock-keywords))
  (set (make-local-variable 'indent-line-function) 'metastory2-indent-line)
  ...
  (add-hook 'font-lock-extend-region-functions 'metastory2-font-lock-extend-region)
  
  ;; This sets up comment info
  (set (make-local-variable 'comment-start) "/#")
  (set (make-local-variable 'comment-style) 'multi-line)

  (setq major-mode 'metastory2-mode)
  (setq mode-name "Meta(Story)")
  (run-hooks 'metastory2-mode-hook)
)
...
(defun metastory2-font-lock-extend-region ()
  ; (message "Here in metastory2-font-lock-extend-region")
  (save-excursion
    (goto-char font-lock-beg)
    (let* ((back-limit (- font-lock-beg 2000))
           (future-limit (+ font-lock-beg 2000))
           (found-point (re-search-backward metastory2-comment-start-re back-limit t)))
      (if found-point
          (let ((last-point (re-search-forward metastory2-comment-re future-limit t)))
