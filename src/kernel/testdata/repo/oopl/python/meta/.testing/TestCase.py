...
import StringIO                           # core meta: 642
import cStringIO                          # core meta: 635
import fcntl                              # core meta: 636
import logging                            # core meta: 637
import os                                 # core meta: 638
import pprint                             # core meta: 639
import re                                 # core meta: 640
import shutil                             # core meta: 641
import stat                               # core meta: 643
import sys                                # core meta: 644
import sys                                # core 
import tempfile                           # core meta: 645
import time                               # core meta: 646
import types                              # core meta: 647
import unittest                           # core meta: 648

import fake_filesystem                    # target meta: 651 target: @pyfakefs//:fake_filesystem
import meta.root                          # target meta: 633
...
class TestCase(unittest.TestCase):
  ...
  there is a CppUnit, JSUnit, python unit (unittest), etc. etc.
  ...
  In Meta<javascript>, the above control flow is changed as follows:
   ...
   - The <C>Test class inherits (directly or indirectly) from
     meta.testing.TestCase (which inherits from goog.testing.TestCase),
   ...
   - In <C>_test.js file, after the definition of <C>Test, the following
     code exists:
       ...
       testCase.run();
   - The 'run' method (meta.testing.TestCase.run) does the following:
  ...
  @classmethod
  def setUpClass(cls):
    """The method used by python to perform per-class setup."""
  ...
  def tearDownClass(cls):
    """The method used by python to perform per-class teardown."""
    if cls.Debug:
  ...
  # field _useold : bool
  #   If True, use code from goog.testing.TestCase in all cases, rather
  #   than overriding certain methods with new functionality.

  def useold(self):
    """Returns: bool"""
    return self._useold

  def useoldIs(self, value):
    """Setter for field useold
  ...
  # field _fs : fake_filesystem.FakeFilesystem
  ...
  def __init__(self, method_name):
    """The javascript asserting methods are available in 
      $CLOSURE_ROOT/library/closure/goog-orig/testing/asserts.js
    Note that they are rather odd in having the optional message be the
    first argument of the method (so we have to do some reversal to get the
    more intuitive 'optional message as last arg' idiom.
    ...
    Currently, there are two lifecycle constructs that can appear in
    the 'tests' attribute of a 'class' construct, one for defining the
    initializer, setUp, tearDown and finalizer methods (instance level),
    and another for defining class initialization, SetUp, TearDown, and
    .. class finalization (meta level).
    ...
    meta.testing.TestCase.InstanceCount += 1
    self.uidIs('%s:%d' % (method_name, meta.testing.TestCase.InstanceCount))
  ...
  def setUp(self):
    """The method used by python and javascript to perform per-test setup"""
  ...
  def tearDown(self):
    """The method used by python and javascript to perform per-test teardown"""
  ...
  def status(self):
    ...
    # in meta.testing.TestCase ... this would override the field
  ...
  def Initialize():
    ...
    TestCase.WriteGoldens = os.getenv('WRITE_GOLDENS', '') == 'true'
  ...
  def raises(self, func, *args, **kwds):
